...
tool. These lists are published to the prlxdisc.org DNS name.

The nodes in the lists are found by crawling the Parallax node discovery DHT. The entire
output of the crawl is available in the `all.json` file. We create lists for specific
blockchain networks by filtering `all.json` according to the ["parallax" ENR entry value][parallax-entry]
...
[parallax-entry]: https://github.com/ethereum/devp2p/blob/master/enr-entries/eth.md
